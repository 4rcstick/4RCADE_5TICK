...
<br><br>
  *It is recommended for users with large mame collections to manually sort them into subfolders.  4RCADE 5TICK can currenly only auto-sort roms located in the root roms folder into a generic "Games" folder, that is still searchable with the search box.  Auto-sorting into folders into categories (via catver.ini) is planned for a later release.*
