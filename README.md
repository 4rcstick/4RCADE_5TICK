...
_It is recommended for users with large ROM collections to manually sort them into subfolders. 4RCADE 5TICK can currently only auto-sort roms located in the root roms folder into a generic "Games" folder, which is still searchable with the search box. Auto-sorting into folders (via catver.ini) is planned for a later release._
